    ...
    save = async (productoAAgregar) => {
        try {
            ...
            if (title != undefined && price != undefined && thumbnail != undefined && descripcion != undefined && stock != undefined && codigo != undefined) {
                ...
                const productoAgregado = await this.dataBase.create({ timestamp: new Date(), title: title, price: price, thumbnail: thumbnail, descripcion: descripcion, stock: stock, codigo: codigo })
                const newData = JSON.parse(JSON.stringify(productoAgregado))
                return newData
            }
            return 1
